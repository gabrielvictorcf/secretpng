...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
		if bytes.len() < 8 {
			return Err(Error::from("Invalid PNG file"));
		}

		let header = bytes[..8].try_into().unwrap();
		let mut bytes = &bytes[8..];
		if header != Png::STANDARD_HEADER {
			return Err(Error::from("Invalid file header"));
		}

		// Read given bytes until exaustion
		let mut chunks = Vec::new();
		while !bytes.is_empty() {
			let chunk = Chunk::try_from(bytes).expect("Invalid file: Corrupted png!");
			let offset = (chunk.length()+12) as usize;
			bytes = &bytes[offset..];
